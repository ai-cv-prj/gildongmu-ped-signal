...
## 현재 상태 — 재분할 모델 학습·평가 완료 (2026-09-21)

사용자가 실행한 새 분할 검출기 학습은 50 epoch를 완료했다. 최고 validation mAP50–95는
48 epoch이며, 완료 audit와 해시가 일치하는 `best.pt`로 새 val 3,681장·test 3,680장을 평가했다.
전체 mAP50–95는 val 84.41%, test 84.10%다. confidence=0.4에서 test 신호등 precision은
87.09%, recall은 96.41%, 작은 신호등 recall은 96.24%다. 횡단보도 precision은 97.65%,
recall은 97.30%다. 복수 신호등 이미지 30장의 신호등 recall은 82.81%로 전체보다 낮다.

이번 test는 이전 test 45장과 구성이 달라 이전 55.53%와 직접 비교해 개선 폭으로 해석하지 않는다.
색상 분류기 재학습과 실제 앱 촬영 정답 평가는 여전히 미실행이다. 새 학습은 추가로 실행하지 않았다.
자세한 조건·검출 수·해석·재실행 명령은 [재분할 모델 평가](RESPLIT_EVALUATION_20260921.md),
수치 사본은 [2026-09-21 평가 기록](reports/20260921/README.md)에 있다.

## 당시 상태 — 앱 화질 재학습·평가·재분할 (2026-09-19)
